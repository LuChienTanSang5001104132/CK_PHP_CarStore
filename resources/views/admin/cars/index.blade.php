                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="p-4">
                            @if($car->featured_image)
                                ...
                                alt="" class="w-20 h-12 object-contain bg-gray-50 rounded-xl shadow-sm">
                            @else
                        ...
                        <td class="p-4">
                            <div class="flex items-center justify-center gap-3">
                                <a href="{{ route('admin.cars.edit', $car->id) }}" class="text-blue-600 hover:text-blue-900 font-semibold bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors">Sửa</a>
                                
                                <form action="{{ route('admin.cars.destroy', $car->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa chiếc xe này?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-900 font-semibold bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors">Xóa</button>
                                </form>
                            </div>
                        </td>
                    </tr>
